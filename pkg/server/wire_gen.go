...
func Initialize(cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	correlationsService, err := correlations.ProvideService(sqlStore, routeRegisterImpl, service15, accessControl, inProcBus, quotaService, cfg, eventualRestConfigProvider, featureToggles)
	if err != nil {
		return nil, err
	}
	...
	correlationAppProvider := correlation.RegisterApp(cfg, correlationsService)
	...
	queryAPIBuilder, err := query2.RegisterAPIService(featureToggles, apiserverService, service15, pluginstoreService, accessControl, middlewareHandler, plugincontextProvider, registerer, tracingService, legacyDataSourceLookup)
	if err != nil {
		return nil, err
	}
