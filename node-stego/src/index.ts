...
// Result interface for decoded messages
...
// Image data interface for both Node.js and browser
...
export function textToData(text: string): Uint8Array {
  return new TextEncoder().encode(text);
}

export function dataToText(data: Uint8Array): string | null {
  try {
...
function setBit(value: number, position: number, bit: number): number {
  if (bit === 1) {
...
export function encodeMessage(data: Uint8Array): Uint8Array {
...
export function decodeMessage(raw: Uint8Array): Uint8Array | null {
  // Check magic header
...
export function embedBitsInTile(tile: Uint8Array, width: number, height: number, bits: number[]): Uint8Array {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (g > result[pixelIdx + brightestChannel]) brightestChannel = 1;
      if (b > result[pixelIdx + brightestChannel]) brightestChannel = 2;

      // Safety check for light pixels
      if (r > LIGHT && g > LIGHT && b > LIGHT && result[pixelIdx + brightestChannel] > 250) {
        const safeChannels = [];
        if (r < 250) safeChannels.push(0);
        if (g < 250) safeChannels.push(1);
        if (b < 250) safeChannels.push(2);
...
export function extractBitsFromTile(tile: Uint8Array, width: number, height: number, lengthBits: number): number[] {
  const bits: number[] = [];
  let bitIdx = 0;
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      let brightestChannel: number;
      if (r > LIGHT && g > LIGHT && b > LIGHT) {
        const nonMaxed = [];
        if (r < 253) nonMaxed.push({idx: 0, val: r});  //253 because can add 2 (bitshift in position 1) and still avoid overflow
        if (g < 253) nonMaxed.push({idx: 1, val: g});
        if (b < 253) nonMaxed.push({idx: 2, val: b});
        ...
        brightestChannel = 0;
        if (g > tile[pixelIdx + brightestChannel]) brightestChannel = 1;
        if (b > tile[pixelIdx + brightestChannel]) brightestChannel = 2;
      }
...
export function encodeImageData(imageData: ImageData, text: string): ImageData {
...
export function decodeImageData(imageData: ImageData): DecodeResult | null {
  ...
  const detectedTiles: Array<{x: number, y: number}> = [];
...
export function decodeImageDataCropped(imageData: ImageData): DecodeResult | null {
  const { data, width, height } = imageData;
  ...
  const tryDecodeAtPosition = (startX: number, startY: number): string | null => {
  ...
  const validateAlignment = (offsetX: number, offsetY: number, expectedMessage: string): DecodeResult | null => {
    const detectedTiles: Array<{x: number, y: number}> = [];
    let votes = 0;
