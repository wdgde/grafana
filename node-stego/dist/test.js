...
async function runTests() {
    ...
    try {
        ...
        await (0, node_1.encodePngFile)(inputImage, outputImage, testMessage);
        // Test decoding
        console.log('🔍 Decoding message from image...');
        const decodedMessage = await (0, node_1.decodePngFile)(outputImage, visualizationImage);
...
async function runCroppedTests(encodedImagePath, expectedMessage) {
    ...
    for (let i = 1; i <= numTests; i++) {
        try {
            ...
            const cropInfo = await (0, node_1.randomlyCropImage)(encodedImagePath, croppedImagePath, 200);
            // Try to decode the cropped image using the cropped decoder
            const decodedMessage = await (0, node_1.decodePngFileCropped)(croppedImagePath, croppedVisualizationPath);
    ...
    try {
        ...
        const cropInfo = await (0, node_1.randomlyCropImage)(encodedImagePath, aggressiveCroppedPath, 150);
        const decodedMessage = await (0, node_1.decodePngFileCropped)(aggressiveCroppedPath, aggressiveVisualizationPath);
        if (decodedMessage === expectedMessage) {
...
async function runDarkLightImageTests() {
    ...
    for (const testImage of testImages) {
        try {
            ...
            await (0, node_1.encodePngFile)(testImage.name, encodedPath, testMessage);
            console.log(`   🔍 Decoding message from ${testImage.type} image...`);
            const decodedMessage = await (0, node_1.decodePngFile)(encodedPath, visualizationPath);
            if (decodedMessage === testMessage) {
...
async function runCroppedTestsForImage(encodedImagePath, expectedMessage, imageType) {
